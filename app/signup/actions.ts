...
export async function signUp(
  ...
  const { data, error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      // Carried in user metadata so provisioning works whenever the first
      // authenticated request happens — even after email confirmation.
      data: {
  ...
  // Session live: provision immediately as the authenticated user.
