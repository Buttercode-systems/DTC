...
(
  ...
  jsonb_build_object(
    'statuses', jsonb_build_array(
      'New Request','Needs Triage','Waiting for Owner Approval','Supplier Quote Needed',
      'Waiting for Supplier','Scheduled','In Progress','Completion Proof Needed','Completed','Cancelled'
    ),
...
(
  ...
  jsonb_build_object(
    'statuses', jsonb_build_array(
      'New Booking','Needs Confirmation','Confirmed','Reschedule Requested','Waiting for Client',
      'Front Desk Follow-up','Completed','No-show Follow-up','Cancelled'
    ),
    'closed_statuses', jsonb_build_array('Completed','Cancelled'),
...
(
  ...
  jsonb_build_object(
    'statuses', jsonb_build_array(
      'New Member','Onboarding','Active','Attendance Risk','Payment Follow-up',
      'Churn Risk','Reactivation Due','Reactivated','Inactive','Cancelled'
    ),
...
begin
  ...
  if p_department <> 'core' and v_template_key is null then
    raise exception 'workflow_template_not_found';
  end if;

  insert into public.businesses(
    owner_id,name,industry,managed_by_tad,primary_contact_name,primary_contact_email,service_status
  ) values (
  ...
  insert into public.service_engagements(
    business_id,department,service_level,status,assigned_operator,start_date,next_review_date,template_key
  ) values (
    v_business.id,p_department,p_service_level,'onboarding',v_uid,current_date,current_date+7,v_template_key
  ) returning * into v_engagement;

  insert into public.actions(
    business_id,key,kind,title,detail,priority,due_date,status,assigned_to,source
  ) values (
    v_business.id,'service_setup:'||v_engagement.id::text,'service_setup',
