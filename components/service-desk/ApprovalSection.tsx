        ...
        <div className="space-y-4">
          {pending.map((approval) => (
            <article
              ...
              key={approval.id}
              className="scroll-mt-24 border border-rule bg-card p-5 shadow-card"
            >
                ...
                <div>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    {approval.amount !== null && (
                      <span className="stamp text-base">{money(approval.amount)}</span>
                    )}
                  </div>
                  {approval.detail && (
                    <p className="mt-3 text-sm leading-6 text-faint">{approval.detail}</p>
                  )}
                ...
                {canManage ? (
                  <form action={decideClientApproval} className="grid gap-3 border border-rule bg-paper p-4">
                    ...
                    <label className="text-sm font-semibold">
                      ...
                      <textarea
                        ...
                        placeholder="Optional reason, limit or instruction"
                      />
                    </label>
                    <div className="grid grid-cols-2 gap-2">
                      <button name="decision" value="approved" className="btn-primary !px-3 !py-2 text-sm">
                        Approve
                      </button>
                      <button name="decision" value="rejected" className="btn-secondary !px-3 !py-2 text-sm">
                        Reject
                      </button>
                    </div>
                  </form>
                  ...
                  <div className="border border-rule bg-paper p-4 text-sm">
                    ...
                    <p className="mt-2 text-faint">
                      You can review the context, but your current workspace role cannot approve or reject this request.
                    </p>
                  </div>
                )}
              </div>
